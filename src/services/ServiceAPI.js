...
export const postGoogleLogin = async acessToken => {
    let res;
    try {
        res = await axiosInstance.post(googlelogin, { access_token: acessToken });
    } catch (err) {
        throw new Error(`In postGoogleLogin: ${err}`);
    }
...
export const getUser = async () => {
    const { jwtAuthToken, accessToken, userId } = getUserAuthData();
    let res;
    try {
        res = await axiosInstance.get(`api/user/${userId}/profile`, { params: { jwtAuthToken, access_token: accessToken } });
    } catch (error) {
...
export const addRecipeToPlanner = async (recipeId, day) => {
    ...
    let res;
    try {
        res = await axiosInstance.post(`api/user/${userId}/planner/${day}`, { recipeId }, { params: { jwtAuthToken, access_token: accessToken } });
    } catch (error) {
...
export const removeRecipeFromPlanner = async (recipeId, day) => {
    ...
    let res;
    try {
        res = await axiosInstance.delete(`api/user/${userId}/planner/${day}/${recipeId}`, { params: { jwtAuthToken, access_token: accessToken } });
    } catch (error) {
        throw new Error(`In removeRecipeFromPlanner: ${error}`);
    }
    if (res.status !== 200) {
        throw new Error(`In removeRecipeFromPlanner: Request failed with ${res.status}`);
    }
    return res.data;
};
...
export const addRecipeToFavourtites = async recipeId => {
    ...
    let res;
    try {
        res = await axiosInstance.post(`api/user/${userId}/favorite`, { recipeId }, { params: { jwtAuthToken, access_token: accessToken } });
    } catch (error) {
        throw new Error(`In addRecipeToFavourtites: ${error}`);
    }
    if (res.status !== 200) {
        throw new Error(`In addRecipeToFavourtites: Request failed with ${res.status}`);
    }
    return res.data;
};
...
export const removeRecipeFromFavourtites = async recipeId => {
    ...
    let res;
    try {
        res = await axiosInstance.delete(`api/user/${userId}/favorite/${recipeId}`, { params: { jwtAuthToken, access_token: accessToken } });
    } catch (error) {
        throw new Error(`In removeRecipeFromFavourtites: ${error}`);
    }
    if (res.status !== 200) {
        throw new Error(`In removeRecipeFromFavourtites: Request failed with ${res.status}`);
    }
    return res.data;
};
